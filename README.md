...
## FWRLM quick start: the FireWorks RocketLauncher Mananger
...
SSH_HOST:             address of MongodDB host accepting SSH connections
SSH_USER:             user name for establishing SSH connection to MongoDB host
SSH_KEY:              /path/to/mongodb/ssh/key/ie/some/id_rsa
MONGODB_PORT_REMOTE:  27017 # that's the MongoDB port at the MongoDB host
MONGODB_PORT_LOCAL:   a port number certainly unused on your machine
```

Make sure to adapt the configuration to your needs. Next, understand what
...
sub-command, i.e. `fwrlm --verbose config reset`. Many `fwrlm` commands will
not display any output otherwise.

Check whether the `FWRLM_config.yaml` is parsed correclty with
...
$ render inspect
╒══════════════════════════════╤════════════════════╤═══════════════════╤═════════╤════════════════╤════════════╤══════════════╤══════════════╤═══════════════╤══════════════════╤═════════════╕
│                              │ MONGODB_PORT_LOCAL │ FW_AUTH_FILE_NAME │ MACHINE │ FIREWORKS_USER │ LOGDIR_LOC │ MONGODB_HOST │ FIREWORKS_DB │ FIREWORKS_PWD │ FW_CONFIG_PREFIX │ WEBGUI_PORT │
├──────────────────────────────┼────────────────────┼───────────────────┼─────────┼────────────────┼────────────┼──────────────┼──────────────┼───────────────┼──────────────────┼─────────────┤
